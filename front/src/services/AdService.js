...
const getAdDetail = (id) => {
    return getFetch(`${API_URL}ads/${id}`)
        .then(res => {
            if (!res.success) {
                return res;
            } else {
...
const searchAds = (query) => {
    ...
    const url = query === 'undefined' || query === ''  ? `${API_URL}ads` : `${API_URL}ads/?${query}`;
    
    console.log('query', query);
    console.log('url', `${API_URL}ads`);

    return getFetch(`${API_URL}ads`)
